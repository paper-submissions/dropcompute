...
def remap_segmented_model_parameters(model_dict: dict, config):
    """Remaps model_dict keys from MLPERF configuration."""
    res_dict = collections.OrderedDict()
    for key in model_dict:
        if any(k in key for k in ('N_Weight', 'iterCnt', 'M_Weight', 'N_Grad',
                                  'M_Grad')):
            continue
        elif any(k in key for k in ('intermediate.dense_act',
                                    'pooler.dense_act', 'transform.dense_act')):
            new_k = key
        elif any(k in key for k in ('intermediate.dense', 'pooler.dense',
                                    'transform.dense')):
            new_k = key.replace("dense", "dense_act")
        else:
            new_k = key
        res_dict[new_k] = model_dict[key]
    model_dict.clear()
    return res_dict
...
def prepare_model_and_optimizer(args, device):
    ...
    if not args.resume_from_checkpoint:
        ...
        if not args.init_checkpoint:
            ...
            checkpoint_remapped = remap_segmented_model_parameters(
                checkpoint['model'], config)
            if utils.is_main_process():
                print('verifying all keys are mapped correctly.')
            for key in model.state_dict().keys():
                if key not in checkpoint_remapped and utils.is_main_process():
                    print('WARNING: model key is missing:', key)
        ...
        if utils.is_main_process():
            print(f'Checkpoint load - resume step from {args.resume_step}')
            print(f'Checkpoint load - global step is set to {global_step}')
    ...
    if args.resume_from_checkpoint and 'optimizer' in checkpoint:
        if utils.is_main_process():
            print('Found optimizer state in checkpoint. Overriding checkpoint'
                  ' hyperparameters.')
        if args.phase2 or args.init_checkpoint:
            ...
            for idx, _ in enumerate(checkpoint['optimizer']['param_groups']):
                if utils.is_main_process():
                    if 'step' in checkpoint['optimizer']['param_groups'][idx]:
                        print('global_step', checkpoint['optimizer']['param_groups'][idx]['step'], global_step,)
                    if 't_total' in checkpoint['optimizer']['param_groups'][idx]:
                        print('max_steps', checkpoint['optimizer']['param_groups'][idx]['t_total'], args.max_steps)
                    if 'warmup' in checkpoint['optimizer']['param_groups'][idx]:
                        print('warmup_proportion', checkpoint['optimizer']['param_groups'][idx]['warmup'], args.warmup_proportion)
                    if 'lr' in checkpoint['optimizer']['param_groups'][idx]:
                        print('learning_rate', checkpoint['optimizer']['param_groups'][idx]['lr'], args.learning_rate)
        ...
        for f_id in range(f_start_id + 1, len(files)):
            ...
            for batch in train_iter:  # delayed update loop
                ...
                try:
                    ...
                    if not is_optimizer_step:
                        divisor = accumulation_stopped
                        global_drop_count += args.gradient_accumulation_steps - computed_batch_size / local_batch_size
                ...
                if global_step >= args.steps_this_run or training_steps % (
                    ...
                    if global_step >= args.steps_this_run or timeout_sent:
                        ...
                        drop_count = torch.tensor([global_drop_count],
                                                  dtype=torch.float32).to(
                            device)
                        torch.distributed.all_reduce(drop_count)
                        if utils.is_main_process():
                            print(f'total drops - {drop_count.item()}/{training_steps * utils.get_world_size()} ({100 * drop_count.item() / (training_steps * utils.get_world_size())}%)')
