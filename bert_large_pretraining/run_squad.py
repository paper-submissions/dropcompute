...
def main():
    ...
    if args.do_train:
        ...
        elif args.use_fused_adam:
            try:
                from habana_frameworks.torch.hpex.optimizers import FusedAdamW
            except ImportError:
                raise ImportError("Please install habana_torch.")
            optimizer = FusedAdamW(optimizer_grouped_parameters, lr=args.learning_rate, bias_correction=False, weight_decay=0.0)
            scheduler = LinearWarmUpScheduler(optimizer, warmup=args.warmup_proportion, total_steps=num_train_optimization_steps)
        else:
    ...
    if args.do_train:
        ...
        if args.use_habana:
            ...
            FusedNorm = FusedClipNorm(model.parameters(), 1.0)
            ...
            for step, batch in enumerate(train_iter):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if args.fp16 :
                        ...
                        scheduler.step()
                    if args.use_habana:
                        if args.per_param_grad_clipping:
                            # gradient clipping is done internally in BertAdam optimizer on a per param
                            # basis. Hence do per param clipping before calling FusedAdamW opt.
                            for group in optimizer.param_groups:
                                for p in group['params']:
                                    FusedNorm.clip_norm(p)
                        if args.use_fused_adam:
                            # modify learning rate with special warm up for BERT which FusedAdamW doesn't do
                            scheduler.step()
                    optimizer.step()
        ...
        with torch.autocast(device_type="hpu", dtype=torch.bfloat16, enabled=args.use_autocast):
            if final_loss is None and last_step != 0:
                for loss_t in loss_list:
                    average_loss += loss_t.item()
