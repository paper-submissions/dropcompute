...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, output_layer_norm_input, input_tensor):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if not self.layer_norm_large_model:
            hidden_states = self.LayerNorm(hidden_states+input_tensor)
            attention_layer_norm_input = 0
        else:
            attention_layer_norm_input = hidden_states+output_layer_norm_input
            hidden_states = self.LayerNorm(attention_layer_norm_input)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, output_layer_norm_input, attention_mask):
        ...
        attention_output, layer_norm_input = self.output(self_output, output_layer_norm_input, input_tensor)
    ...
    def checkpointed_forward(self, hidden_states, layer_norm_input, attention_mask, interval):
        def custom(start, end):
            def custom_forward(*inputs):
                ...
                x_, layer_norm_input_, attention_mask_ = inputs
                for layer in layers:
                    x_, layer_norm_input_ = layer(x_, layer_norm_input_, attention_mask_)
                return x_, layer_norm_input_
...
class BertPreTrainedModel(nn.Module):
    ...
    def checkpoint_activations(self, enabled, interval=1):
        def _apply_flag(module):
            if hasattr(module, "_checkpoint_activations"):
                module._checkpoint_activations = enabled
                module._checkpoint_activations_interval = interval
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, masked_lm_labels=None, next_sentence_labels=None):
