# DeepSpeed BERT-1.5B for PyTorch
This folder contains scripts to pre-train BERT-1.5B model using DeepSpeed on Habana Gaudi device to achieve an accurate training of a large scale model. To obtain model performance data, refer to the [Habana Model Performance Data page](https://developer.habana.ai/resources/habana-training-models/#performance).
...
- Multi-server data parallel with Zero1 and BF16.
...
required for multi-server demo.
...
- For AWS DL1 users it is recommended to use the below `~/.deepspeed_env` configuration:
  ...
  ```
  **Note:** Ensure the `DATA_DIR` variable in the [run_deepspeed_bert_1.5b.sh](./scripts/run_deepspeed_bert_1.5b.sh) script contains the correct path to the input dataset.

Run pre-training for Phase 1 on 32 HPUs with automatic drop threshold:
```bash
bash ./scripts/run_deepspeed_bert_1.5b.sh 192 16 -2
```

Run pre-training for Phase 1 on 32 HPUs without DropCompute:
```bash
bash ./scripts/run_deepspeed_bert_1.5b.sh 192 16 -1
```
...
- 32 card training helper script: [run_deepspeed_bert_1.5b.sh](./scripts/run_deepspeed_bert_1.5b.sh)
