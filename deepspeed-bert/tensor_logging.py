...
def save_logged_tensors(tensor_logger: TensorLogger, tensor_logger_path, rank_no):
...
def save_drop_compute_logs(time_logs, log_dir, rank, average_loss_list):
    if log_dir is None:
        log_dir = ''
    with open(join(log_dir, f'compute_logs_{rank}.csv'), 'w') as file:
    ...
    with open(join(log_dir, f'loss_logs_{rank}.csv'), 'w') as file:
        file.write(pd.DataFrame(average_loss_list, columns=(
            'global_step',
            'computed_batch',
            'average_loss',
            'loss_list_length'
