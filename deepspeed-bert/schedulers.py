...
class PolyWarmUpScheduler(LRScheduler):
    ...
    def __init__(self, optimizer, warmup, total_steps, degree=1.0, constant=0.0, last_epoch=-1):
        self.warmup = warmup
        self.total_steps = total_steps
        self.degree = degree
    ...
    def get_lr(self):
        ...
        if progress < self.warmup:
            return [base_lr * progress / self.warmup for base_lr in self.base_lrs]
        elif  progress < (self.warmup + self.constant):
            return [base_lr for base_lr in self.base_lrs]
        else:
            return [base_lr * (((1.0 - progress)/ (1.0 - self.warmup - self.constant)) ** self.degree) for base_lr in self.base_lrs]
