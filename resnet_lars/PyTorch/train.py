...
def train_one_epoch(lr_scheduler, model, criterion, optimizer, data_loader, device, epoch,
                    print_freq, args, device_random, apex=False, warmup=False):
    model.train()
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if device_random.randint(1, 100) <= args.drop_rate and epoch > 1:
            print(f'rank {dist.get_rank()} dropped at epoch {epoch} and step'
                  f' {step_count}')
            hook_handles = []
            dropped = True
...
def warmup(model_for_train, model_for_eval, device, criterion, optimizer, args, data_loader_type, pin_memory_device, pin_memory):
    ...
    dataset, dataset_test, train_sampler, test_sampler = load_data(
        f'{args.output_dir}/resnet_synth_data/train',
        f'{args.output_dir}/resnet_synth_data/val', args=args, synthetic=True)
    ...
    data_loader_test = data_loader_type(
        ...
        num_workers=args.workers, pin_memory=pin_memory, pin_memory_device=pin_memory_device)
    device_random = random.Random(dist.get_rank())
    train_one_epoch(None, model_for_train, criterion, optimizer, data_loader, device, 0,
                    1, args, device_random, apex=args.apex, warmup=True)
...
def main(args):
    ...
    steps_per_epoch = ceil(NUM_IMAGES['train'] / args.world_size / args.batch_size)
    steps_per_eval = ceil(NUM_IMAGES['validation'] / args.world_size / args.batch_size)
    ...
    lr_scheduler = PolynomialDecayWithWarmup(optimizer,
                                             ...
                                             mlperf_mllog=mlperf_mllog if utils.is_main_process() else None,
                                             mlperf_mlloger=mlperf_mlloger if utils.is_main_process() else None,
                                             opt_name='lars')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(lr_scheduler, model_for_train, criterion, optimizer, data_loader,
                        device, epoch, print_freq=train_print_freq, args=args, device_random=device_random, apex=args.apex)
        ...
        if epoch == next_eval_epoch:
            ...
            if next_eval_epoch < args.epochs and utils.is_main_process():
                mlperf_mlloger.start(
                    key=mlperf_mllog.constants.BLOCK_START,
                    value=None,
                    metadata={
