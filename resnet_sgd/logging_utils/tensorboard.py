...
def init(log_dir: Optional[str] = None, comment: Optional[str] = '') -> None:
    """Creates a TensorBoard logger that writes events an event file.
    Args:
        log_dir: Save directory location. Default is runs/**CURRENT_DATETIME**.
        comment: Comment log_dir suffix appended to the default ``log_dir``.
         If ``log_dir`` is assigned, this argument has no effect.
    """
    if not log_dir:
...
def log_hyper_parameters(hyper_param: Optional[Mapping[str, Any]] = None):
    ...
    _root.writer.add_hparams(
        _root.hparams, {'train_loss': 0}, run_name='hparams')
